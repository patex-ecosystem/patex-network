...
import (
	...
	"github.com/patex-ecosystem/patex-network/pt-chain-ops/util"
	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				...
				Usage: "Address of the PatexPortal on L1",
			},
			&cli.StringFlag{
		...
		Action: func(ctx *cli.Context) error {
			...
			proveWithdrawals := func(wd *crossdomain.LegacyWithdrawal, bf *big.Int, i int) {
				...
				proven, err := contracts.PatexPortal.ProvenWithdrawals(&bind.CallOpts{}, hash)
				if err != nil {
				...
				if proven.Timestamp.Cmp(common.Big0) == 0 {
					log.Info("Proving withdrawal to PatexPortal")
					...
					log.Info("Withdrawal already proven to PatexPortal")
				}
			}
			...
			finalizeWithdrawals := func(wd *crossdomain.LegacyWithdrawal, bf *big.Int, i int) {
				...
				isFinalized, err := contracts.PatexPortal.FinalizedWithdrawals(&bind.CallOpts{}, hash)
				if err != nil {
...
func proveWithdrawalTransaction(c *contracts, cl *util.Clients, opts *bind.TransactOpts, withdrawal *crossdomain.Withdrawal, bn *big.Int) error {
	...
	tx, err := c.PatexPortal.ProveWithdrawalTransaction(
		opts,
		wdTx,
		l2OutputIndex,
		outputRootProof,
...
func finalizeWithdrawalTransaction(
	...
	// Finalize withdrawal
	tx, err := c.PatexPortal.FinalizeWithdrawalTransaction(
		opts,
		wdTx,
	)
	if err != nil {
...
type contracts struct {
	PatexPortal            *bindings.PatexPortal
...
func newContracts(ctx *cli.Context, l1Backend, l2Backend bind.ContractBackend) (*contracts, error) {
	patexPortalAddress := ctx.String("patex-portal-address")
	if len(patexPortalAddress) == 0 {
		return nil, errors.New("PatexPortal address not configured")
	}
	patexPortalAddr := common.HexToAddress(patexPortalAddress)

	portal, err := bindings.NewPatexPortal(patexPortalAddr, l1Backend)
	if err != nil {
		return nil, err
	}
