...
func TestBuildL1DeveloperGenesis(t *testing.T) {
	...
	require.NoError(t, err)
	portal, err := bindings.NewPatexPortal(predeploys.DevPatexPortalAddr, sim)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, predeploys.DevPatexPortalAddr, portalAddr)
	...
	factory, err := bindings.NewPatexMintableERC20(predeploys.DevPatexMintableERC20FactoryAddr, sim)
	require.NoError(t, err)
