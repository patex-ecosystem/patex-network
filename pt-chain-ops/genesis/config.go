...
// DeployConfig represents the deployment configuration for Patex
type DeployConfig struct {
	...
	// GUARDIAN account in the PatexPortal
	...
	// PatexPortal proxy address on L1
	PatexPortalProxy common.Address `json:"patexPortalProxy"`
...
func (d *DeployConfig) Check() error {
	...
	if d.PatexPortalProxy == (common.Address{}) {
		return fmt.Errorf("%w: PatexPortalProxy cannot be address(0)", ErrInvalidDeployConfig)
	}
...
func (d *DeployConfig) GetDeployedAddresses(hh *hardhat.Hardhat) error {
	...
	if d.PatexPortalProxy == (common.Address{}) {
		patexPortalProxyDeployment, err := hh.GetDeployment("PatexPortalProxy")
		if err != nil {
			return err
		}
		d.PatexPortalProxy = patexPortalProxyDeployment.Address
	}
...
func (d *DeployConfig) InitDeveloperDeployedAddresses() error {
	...
	d.PatexPortalProxy = predeploys.DevPatexPortalAddr
...
func (d *DeployConfig) RollupConfig(l1StartBlock *types.Block, l2GenesisBlockHash common.Hash, l2GenesisBlockNumber uint64) (*rollup.Config, error) {
	if d.PatexPortalProxy == (common.Address{}) {
		return nil, errors.New("PatexPortalProxy cannot be address(0)")
	}
	if d.SystemConfigProxy == (common.Address{}) {
	...
	return &rollup.Config{
		...
		DepositContractAddress: d.PatexPortalProxy,
...
func NewL2ImmutableConfig(config *DeployConfig, block *types.Block) (immutables.ImmutableConfig, error) {
	...
	immutable["PatexMintableERC721Factory"] = immutables.ImmutableValues{
