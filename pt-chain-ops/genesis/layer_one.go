...
import (
	...
	"github.com/patex-ecosystem/patex-network/pt-chain-ops/state"
)
...
var (
	...
	proxies = []string{
		...
		"PatexPortalProxy",
		"PatexMintableERC20FactoryProxy",
	}
...
// all of the state required for an Patex network to function.
func BuildL1DeveloperGenesis(config *DeployConfig) (*core.Genesis, error) {
	...
	portalABI, err := bindings.PatexPortalMetaData.GetAbi()
	if err != nil {
		return nil, err
	}
	// Initialize the PatexPortal without being paused
	data, err := portalABI.Pack("initialize", false)
	if err != nil {
		return nil, fmt.Errorf("cannot abi encode initialize for PatexPortal: %w", err)
	}
	if _, err := upgradeProxy(
		backend,
		opts,
		depsByName["PatexPortalProxy"].Address,
		depsByName["PatexPortal"].Address,
		data,
	); err != nil {
		return nil, fmt.Errorf("cannot upgrade PatexPortalProxy: %w", err)
	}
	...
	if lastUpgradeTx, err = upgradeProxy(
		backend,
		opts,
		depsByName["PatexMintableERC20FactoryProxy"].Address,
		depsByName["PatexMintableERC20Factory"].Address,
		nil,
	); err != nil {
		return nil, err
	}
	...
	for _, dep := range deployments {
		...
		for iter.Next() {
			...
			if depAddr == predeploys.DevPatexPortalAddr && key == portalMeteringSlot {
...
func deployL1Contracts(config *DeployConfig, backend *backends.SimulatedBackend) ([]deployer.Deployment, error) {
	...
	constructors = append(constructors, []deployer.Constructor{
		...
		{
			// The implementation of the PatexPortal is deployed
...
func l1Deployer(backend *backends.SimulatedBackend, opts *bind.TransactOpts, deployment deployer.Constructor) (*types.Transaction, error) {
	...
	case "PatexPortal":
		_, tx, _, err = bindings.DeployPatexPortal(
			opts,
			backend,
			deployment.Args[0].(common.Address),
	...
	case "PatexMintableERC20Factory":
		_, tx, _, err = bindings.DeployPatexMintableERC20Factory(
			opts,
			backend,
