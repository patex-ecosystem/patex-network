...
Each epoch may contain a variable number of L2 blocks (one every `l2_block_time`, 2s on Patex), at the discretion of
...
- `min_l2_timestamp <= block.timestamp <= max_l2_timestamp`, where
  ...
  - `block.timestamp = prev_l2_timestamp + l2_block_time`
    ...
    - `l2_block_time` is a configurable parameter of the time between L2 blocks (on Patex, 2s)
...
through an unsafe source such as the sequencer publishing it via the p2p network, then it is patexally processed as
...
zk-proof to finalize. Patex-rollup outputs like withdrawals on L1 are only labeled "finalized" after passing a week
