...
The Ethereum Node Record (ENR) for an Patex rollup node must contain the following values, identified by unique keys:
...
The discovery process in Patex is a pipeline of node records:
...
`/Patex/chain_id/hardfork_version/Name`
...
The pt-node implements a similar request-response encoding for its sync protocols as the L1 ethereum Beacon-Chain.
...
The req-resp protocols served by the pt-node all have `/ProtocolPrefix` set to `/opstack/req`.
...
- `/MessageName` is `/block_by_number/<chain-id>` where `<chain-id>` is set to the pt-node L2 chain ID.
- `/SchemaVersion` is `/0`
