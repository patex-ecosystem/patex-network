...
properties of the message in its storage. Withdrawals are finalized on L1 via a call to the `PatexPortal`
...
- [The Patex Portal Contract](#the-Patex-portal-contract)
...
1. A [relayer][g-relayer] submits the required inputs to the `PatexPortal` contract. The relayer need
...
1. The `PatexPortal` contract retrieves the output root for the given block number from the `L2OutputOracle`'s
...
1. Once the challenge period has passed, a relayer submits the withdrawal transaction once again to the
   `PatexPortal` contract. Again, the relayer need not be the same entity which initiated the withdrawal on L2.
1. The `PatexPortal` contract receives the withdrawal transaction data and verifies that the withdrawal has
...
- on L1, the withdrawal sender's address is accessed by calling the `l2Sender()` function on the `PatexPortal`
  contract.
...
## The Patex Portal Contract

The Patex Portal serves as both the entry and exit point to the Patex L2. It is a contract which inherits from
the [PatexPortal](./deposits.md#deposit-contract) contract, and in addition provides the following interface for
withdrawals:
...
[`WithdrawalTransaction` type]: https://github.com/ethereum-Patex/Patex/blob/6c6d142d7bb95faa11066aab5d8aed7187abfe38/packages/contracts-bedrock/contracts/libraries/Types.sol#L76-L83
[`OutputRootProof` type]: https://github.com/ethereum-Patex/Patex/blob/6c6d142d7bb95faa11066aab5d8aed7187abfe38/packages/contracts-bedrock/contracts/libraries/Types.sol#L33-L38
