...
type Server struct {
	...
	rpcPath        string
	healthzPath    string
	httpRecorder   ptmetrics.HTTPRecorder
...
func WithHTTPRecorder(recorder ptmetrics.HTTPRecorder) ServerOption {
	return func(b *Server) {
...
func NewServer(host string, port int, appVersion string, opts ...ServerOption) *Server {
	...
	bs := &Server{
		...
		rpcPath:        "/",
		healthzPath:    "/healthz",
		httpRecorder:   ptmetrics.NoopHTTPRecorder,
		httpServer: &http.Server{
			Addr: endpoint,
		},
		log: log.Root(),
...
func (b *Server) Start() error {
	...
	handler = ptmetrics.NewHTTPRecordingMiddleware(b.httpRecorder, handler)
	handler = ptlog.NewLoggingMiddleware(b.log, handler)
