...
import (
	...
	"github.com/patex-ecosystem/patex-network/pt-node/withdrawals"
...
func init() {
	...
	flag.Parse()
	if os.Getenv("PT_E2E_DISABLE_PARALLEL") == "true" {
...
func TestSystemE2E(t *testing.T) {
	...
	depositContract, err := bindings.NewPatexPortal(predeploys.DevPatexPortalAddr, l1Client)
	require.Nil(t, err)
...
func TestWithdrawals(t *testing.T) {
	...
	defer cancel()
	blockNumber, err := withdrawals.WaitForFinalizationPeriod(ctx, l1Client, predeploys.DevPatexPortalAddr, receipt.BlockNumber)
	require.Nil(t, err)
	...
	defer cancel()
	_, err = withdrawals.WaitForFinalizationPeriod(ctx, l1Client, predeploys.DevPatexPortalAddr, header.Number)
	require.Nil(t, err)
...
func TestFees(t *testing.T) {
	...
	cfg := DefaultSystemConfig(t)
	// TODO: after we have the system config contract and new pt-geth L1 cost utils,
