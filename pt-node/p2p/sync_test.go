...
func TestSinglePeerSync(t *testing.T) {
	...
	srv := NewReqRespServer(cfg, servePayload, metrics.Noptmetrics)
	...
	cl := NewSyncClient(log.New("role", "client"), cfg, hostB.NewStream, receivePayload, metrics.Noptmetrics)
...
func TestMultiPeerSync(t *testing.T) {
	...
	setupPeer := func(ctx context.Context, h host.Host) (*SyncClient, chan *eth.ExecutionPayload) {
		...
		cl := NewSyncClient(log.New("role", "client"), cfg, h.NewStream, receivePayload, metrics.Noptmetrics)
		return cl, received
	}
