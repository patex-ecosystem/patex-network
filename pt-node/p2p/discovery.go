...
// The discovery ENRs are just key-value lists, and we filter them by records tagged with the "ptstack" key,
// and then check the chain ID and version.
type PtStackENRData struct {
	chainID uint64
	version uint64
}

func (o *PtStackENRData) ENRKey() string {
	return "ptstack"
}

func (o *PtStackENRData) EncodeRLP(w io.Writer) error {
...
func (o *PtStackENRData) DecodeRLP(s *rlp.Stream) error {
	b, err := s.Bytes()
	if err != nil {
	...
	// Future "ptstack" entries may contain additional data, and will be tagged with a newer version etc.
	r := bytes.NewReader(b)
...
func (n *NodeP2P) DiscoveryProcess(ctx context.Context, log log.Logger, cfg *rollup.Config, connectGoal uint) {
	...
	for {
		...
		case found := <-randomNodesCh:
			...
			n.ConnectionManager().TagPeer(info.ID, fmt.Sprintf("ptstack-%d-%d", dat.chainID, dat.version), 42)
