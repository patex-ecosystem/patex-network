...
func TestP2PSimple(t *testing.T) {
	...
	hostA, err := confA.Host(testlog.Logger(t, log.LvlError).New("host", "A"), nil, metrics.Noptmetrics)
	require.NoError(t, err, "failed to launch host A")
	defer hostA.Close()
	hostB, err := confB.Host(testlog.Logger(t, log.LvlError).New("host", "B"), nil, metrics.Noptmetrics)
...
func TestP2PFull(t *testing.T) {
	...
	nodeA, err := NewNodeP2P(context.Background(), &rollup.Config{}, logA, &confA, &mockGossipIn{}, nil, runCfgA, metrics.Noptmetrics)
	...
	nodeB, err := NewNodeP2P(context.Background(), &rollup.Config{}, logB, &confB, &mockGossipIn{}, nil, runCfgB, metrics.Noptmetrics)
	require.NoError(t, err)
	defer nodeB.Close()
	hostB := nodeB.Host()
...
func TestDiscovery(t *testing.T) {
	...
	nodeA, err := NewNodeP2P(context.Background(), rollupCfg, logA, &confA, &mockGossipIn{}, nil, runCfgA, metrics.Noptmetrics)
	require.NoError(t, err)
	defer nodeA.Close()
	hostA := nodeA.Host()
	...
	// Start B
	nodeB, err := NewNodeP2P(context.Background(), rollupCfg, logB, &confB, &mockGossipIn{}, nil, runCfgB, metrics.Noptmetrics)
	require.NoError(t, err)
	defer nodeB.Close()
	hostB := nodeB.Host()
	...
	// Start C
	nodeC, err := NewNodeP2P(context.Background(), rollupCfg, logC, &confC, &mockGossipIn{}, nil, runCfgC, metrics.Noptmetrics)
	require.NoError(t, err)
	defer nodeC.Close()
	hostC := nodeC.Host()
	...
	// B should be connected to the bootnode (A) it used (it's a valid patex node to connect to here)
