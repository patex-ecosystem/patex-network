...
import (
	...
	"github.com/patex-ecosystem/patex-network/pt-node/chaincfg"
	"github.com/patex-ecosystem/patex-network/pt-node/cmd/doc"

	"github.com/urfave/cli"

	"github.com/ethereum/go-ethereum/log"

	ptnode "github.com/patex-ecosystem/patex-network/pt-node"
	"github.com/patex-ecosystem/patex-network/pt-node/cmd/genesis"
	"github.com/patex-ecosystem/patex-network/pt-node/cmd/p2p"
	"github.com/patex-ecosystem/patex-network/pt-node/flags"
	"github.com/patex-ecosystem/patex-network/pt-node/heartbeat"
	"github.com/patex-ecosystem/patex-network/pt-node/metrics"
	"github.com/patex-ecosystem/patex-network/pt-node/node"
...
func main() {
	...
	app.Name = "pt-node"
	app.Usage = "Patex Rollup Node"
	app.Description = "The Patex Rollup Node derives L2 block inputs from L1 data and drives an external L2 Execution Engine to build a L2 chain."
...
func RollupNodeMain(ctx *cli.Context) error {
	...
	logCfg := ptlog.ReadCLIConfig(ctx)
	if err := logCfg.Check(); err != nil {
		log.Error("Unable to create the log config", "error", err)
		return err
	}
	log := ptlog.NewLogger(logCfg)
	ptservice.ValidateEnvVars(flags.EnvVarPrefix, flags.Flags, log)
	m := metrics.NewMetrics("default")

	cfg, err := ptnode.NewConfig(ctx, log)
	if err != nil {
		log.Error("Unable to create the rollup node config", "error", err)
		return err
	}
	snapshotLog, err := ptnode.NewSnapshotLogger(ctx)
	if err != nil {
	...
	if cfg.Pprof.Enabled {
		...
		go func() {
			...
			if err := ptpprof.ListenAndServe(pprofCtx, cfg.Pprof.ListenAddr, cfg.Pprof.ListenPort); err != nil {
