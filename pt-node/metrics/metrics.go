...
import (
	...
	ophttp "github.com/patex-ecosystem/patex-network/pt-node/http"
	"github.com/patex-ecosystem/patex-network/pt-service/metrics"
...
// Metrics tracks all the metrics for the pt-node.
type Metrics struct {
...
func NewMetrics(procName string) *Metrics {
	...
	return &Metrics{
		...
		Up: factory.NewGauge(prometheus.GaugeOpts{
			Namespace: ns,
			Name:      "up",
			Help:      "1 if the pt node has finished starting up",
		}),
		...
		RPCClientRequestsTotal: factory.NewCounterVec(prometheus.CounterOpts{
			...
			Name:      "requests_total",
			Help:      "Total RPC requests initiated by the ptnode's RPC client",
		}, []string{
			"method",
		}),
		...
		RPCClientResponsesTotal: factory.NewCounterVec(prometheus.CounterOpts{
			...
			Help:      "Total RPC request responses received by the ptnode's RPC client",
		}, []string{
			"method",
			"error",
		}),
...
// call to the ptnode's RPC server. It bumps the requests metric,
...
var Noptmetrics Metricer = new(noopMetricer)
