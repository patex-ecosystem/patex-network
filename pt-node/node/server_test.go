...
import (
	...
	rpcclient "github.com/patex-ecosystem/patex-network/pt-node/client"
...
func TestOutputAtBlock(t *testing.T) {
	...
	server, err := newRPCServer(context.Background(), rpcCfg, rollupCfg, l2Client, drClient, log, "0.0", metrics.Noptmetrics)
	...
	err = client.CallContext(context.Background(), &out, "patex_outputAtBlock", "0xdcdc89")
	require.NoError(t, err)
...
func TestVersion(t *testing.T) {
	...
	var out string
	err = client.CallContext(context.Background(), &out, "patex_version")
	assert.NoError(t, err)
...
func TestSyncStatus(t *testing.T) {
	...
	err = client.CallContext(context.Background(), &out, "patex_syncStatus")
	assert.NoError(t, err)
