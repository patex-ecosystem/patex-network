...
func CLIFlags(envPrefix string) []cli.Flag {
	return []cli.Flag{
		cli.Uint64Flag{
			...
			Value:  100_000,
			EnvVar: ptservice.PrefixEnvVar(envPrefix, "TARGET_L1_TX_SIZE_BYTES"),
		},
		cli.IntFlag{
			Name:   TargetNumFramesFlagName,
			Usage:  "The target number of frames to create per channel",
			Value:  1,
			EnvVar: ptservice.PrefixEnvVar(envPrefix, "TARGET_NUM_FRAMES"),
		},
		cli.Float64Flag{
			Name:   ApproxComprRatioFlagName,
			Usage:  "The approximate compression ratio (<= 1.0)",
			Value:  0.4,
			EnvVar: ptservice.PrefixEnvVar(envPrefix, "APPROX_COMPR_RATIO"),
		},
		cli.StringFlag{
			Name:   KindFlagName,
			Usage:  "The type of compressor. Valid options: " + strings.Join(KindKeys, ", "),
			EnvVar: ptservice.PrefixEnvVar(envPrefix, "COMPRESSOR"),
			Value:  RatioKind,
		},
	}
}
